...
<table>
<tr>
<td><div class="row1"><?php echo stripcslashes($ticketsval->subject); ?> </div></td>
<td><a href="javascript:closeTicket(<?php echo $ticket_id; ?>)" class="Reply"><?php _e('Close','ydssupport'); ?></a></td>
<td><a href="#gotoreply" class="Reply"><?php _e('GO To Reply','ydssupport'); ?></a></td>
</tr></table>
</div>
...
<div class="threadInfo">
   ...
   <small class="threadCreateTime"><?php echo $thread_modified;?></small>
</div>
<div class="threadMsg">
...
<?php if(count($attachments)){?>
			<div class="threadAttachment">
				...
				foreach ($attachments as $attachment){
					...
					$attachCount++;
				?>
				<a class="attachment_link" target="_blank" href="<?php echo esc_url($attach->attachments_url); ?>" ><img width="90" src="<?php echo esc_url($attach->attachments_url);?>" /></a>
				<?php }?>
			</div>
			<?php }?>
</div>
...
<div class="ticket-row" id="multipleAttachement">
 <div class="attachment-row">  <label>&nbsp;</label> <input name="attachment[]" id="attachfile" type="file">   <span class="dashicons dashicons-plus-alt cursorPointer" id="addissueField" onclick="addMoreattach(event,this);"></span> </div>
</div>
<!--field--><div class="ticket-row">
<label>&nbsp;</label><input type="hidden" name="action" value="responseTicket" />
